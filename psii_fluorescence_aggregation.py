...
def generate_aggregate(csv):
    ...
    sorted_df['ID'] = sorted_df['Label'].str.split('_', expand=True)[0]
    # print(len(sorted_df))

    for image in sorted_df['ID'].unique():

        temp_df = sorted_df[sorted_df['ID']==image]

        try:
            #sorted_df = df[['folder_name', 'Label', 'x', 'y', 'Area', 'Mean', 'Min', 'Max']]
            temp_df['MultiThr'] = multithresh_list
